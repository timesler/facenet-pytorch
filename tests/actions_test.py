...
os.system(
    "jupyter nbconvert --to script --stdout examples/infer.ipynb examples/finetune.ipynb > examples/tmptest.py"
)
os.chdir("examples")
try:
...
for img, idx in dataset:
    ...
    assert (
        img_box - mtcnn_pt.detect(np.array([np.array(img), np.array(img)]))[0]
    ).sum() < 1e-2
    assert (
        img_box - mtcnn_pt.detect(torch.as_tensor([np.array(img), np.array(img)]))[0]
    ).sum() < 1e-2
    ...
    if img_align is not None:
        ...
        aligned_fromfile.append(
            get_image("data/test_images_aligned/{}/1.png".format(name), trans_cropped)
        )
...
for i, ds in enumerate(["vggface2", "casia-webface"]):
    ...
    dists_fromfile = [
        [(emb - e).norm().item() for e in embs_fromfile] for emb in embs_fromfile
    ]
    ...
    total_error_fromfile = (
        torch.tensor(dists_fromfile) - torch.tensor(expected[i])
    ).norm()
...
box_diff = (
    boxes_ref[np.argsort(boxes_ref[:, 1])] - boxes_test[np.argsort(boxes_test[:, 1])]
)
...
if torch.cuda.is_available():
    ...
    box_diff = (
        boxes_ref[np.argsort(boxes_ref[:, 1])]
        - boxes_test[np.argsort(boxes_test[:, 1])]
    )
