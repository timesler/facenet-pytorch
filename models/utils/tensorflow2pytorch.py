...
def get_layer_indices(layer_lookup, tf_layers):
    ...
    for name, value in layer_lookup.items():
        layer_inds[name] = value + [
            [i for i, n in enumerate(tf_layers) if value[0] in n]
        ]
    return layer_inds
...
def load_tf_conv2d(weights, layer, transpose=False):
    ...
    layer.weight.data = (
        torch.tensor(weights).permute(dim_order).view(layer.weight.data.shape)
    )
...
def load_tf_linear(weights, layer):
    ...
    if isinstance(weights, list):
        ...
        weights = weights[0]
    layer.weight.data = (
        torch.tensor(weights).transpose(-1, 0).view(layer.weight.data.shape)
    )
...
def load_tf_repeats(weights, layer, rptlen, subfun):
    if len(weights) % rptlen != 0:
        raise ValueError(
            f"Number of weight arrays ({len(weights)}) not divisible by {rptlen}"
        )
